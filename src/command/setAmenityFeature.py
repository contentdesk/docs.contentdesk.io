...
def main():
    ...
    # DEBUG
    with open("../../output/discover/amenityFeatures/amenityFeaturesEN.json", "w") as file:
        json.dump(amenityFeaturesEN, file, indent=4)
    with open("../../output/discover/amenityFeatures/amenityFeaturesDE.json", "w") as file:
        json.dump(amenityFeaturesDE, file, indent=4)
    with open("../../output/discover/amenityFeatures/amenityFeaturesFR.json", "w") as file:
        json.dump(amenityFeaturesFR, file, indent=4)
    with open("../../output/discover/amenityFeatures/amenityFeaturesIT.json", "w") as file:
        json.dump(amenityFeaturesIT, file, indent=4)
    ...
    with open("../../output/contentdesk/attribute/amenityFeatures/amenityFeatures.json", "w") as file:
        json.dump(akeneoAmenityFeatures, file, indent=4)

    # Save as csv with UTF-8 encoding and replace "None" in every field with empty string
    with open("../../output/contentdesk/attribute/amenityFeatures/akeneoAmenityFeatures.csv", "w", encoding='utf-8') as file:
    ...
    # Compare
    with open("../../docs/schema/amenityFeatures.csv", "w", encoding='utf-8') as file:
        for code, body in akeneoAmenityFeatures.items():
            en = body['labels']['en_US'] if 'en_US' in body['labels'] else ''
            de = body['labels']['de_DE'] if 'de_DE' in body['labels'] else ''
            fr = body['labels']['fr_FR'] if 'fr_FR' in body['labels'] else ''
